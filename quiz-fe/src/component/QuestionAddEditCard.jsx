...
import {validateQuestion} from "../lib/common";
...
const QuestionAddEditCard = ({onQuestionUpdate, ...props}) => {
    ...
    console.log('options',options, 'correctOptions', correctOptions)
    return <Wrapper>
        <Card className='quiz-card'>
            <FormControl className='type'>
                ...
                {(props?.setAdd || props?.setEdit) &&
                    <Button onClick={() => props.setAdd ? props.setAdd(false) : props.setEdit({})}><CloseIcon
                        color='error'/></Button>}
            </FormControl>
            <div className="space">
                <TextField
            ...
            <CardActions className='action-button'>
                ...
                <Fab color="secondary" aria-label="edit" onClick={() => {
                    if (validateQuestion({title, options, type, correctOptions}))
